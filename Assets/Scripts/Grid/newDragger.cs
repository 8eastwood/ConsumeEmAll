...
{
    [SerializeField] private newGridHandler _gridHandler;
    ...
    private void UpdateTargetPosition()
    {
        ...
        Vector3 offsettedPoint = castedPoint + _dragOffset;

        if (_gridHandler.TryGetValidGridCoord(castedPoint, out Vector2 position))
            _targetPosition = offsettedPoint;
    }

    private void MoveShape()
    {
        _selectedObject.TryGetComponent<Rigidbody>(out var rb);
        rb.velocity = (_targetPosition - _selectedObject.transform.position) * 10;
    ...
    private IEnumerator SnappingToGrid()
    {
        ...
        _gridHandler.TryGetValidGridCoord(temporaryObject.transform.position, out Vector2 position);
